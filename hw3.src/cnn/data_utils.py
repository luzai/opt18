...
def choice_lbl(x, y, choices=range(10)):
    mask = np.zeros_like(y)
    for ind, choice in enumerate(choices):
        print('Note: class {} has been map to {}'.format(choice, ind))
        mask += (y == choice)
        y[y == choice] = ind
...
def get_MNIST_data(mnist_dirpath, substract_mean=True,
                   num_training=None, num_validation=500, num_test=None,
    ...
    with open(training_set_labels, "rb") as f:
        ...
        trn_y = np.fromfile(f, dtype=np.uint8)
    ...
    with open(test_set_labels, "rb") as f:
        magic, num = struct.unpack(">II", f.read(8))
        tst_y = np.fromfile(f, dtype=np.uint8)

    trn_x = np.array([img / 255. for img in trn_imgs]).astype(np.float32)
    tst_x = np.array([img / 255. for img in tst_imgs]).astype(np.float32)

    if substract_mean:
        mean_x = trn_x.mean(axis=0)
        trn_x -= mean_x
        tst_x -= mean_x
    trn_x, trn_y = choice_lbl(trn_x, trn_y, choice_classes)
    tst_x, tst_y = choice_lbl(tst_x, tst_y, choice_classes)
    if num_training is not None:
        trn_x = trn_x[:num_training]
        trn_y = trn_y[:num_training]
    if num_test is not None:
        tst_x = tst_x[:num_test]
        tst_y = tst_y[:num_test]
    val_x = trn_x[:num_validation]
    val_y = trn_y[:num_validation]
    return {
        'X_train': trn_x, 'y_train': trn_y,
        'X_val': val_x, 'y_val': val_y,
        'X_test': tst_x, 'y_test': tst_y,
    }
...
def load_imagenet_val(num=None):
    ...
    """
    imagenet_fn = 'cnn/datasets/imagenet_val_25.npz'
    if not os.path.isfile(imagenet_fn):
        print('file %s not found' % imagenet_fn)
        print('Run the following:')
        print('cd cnn/datasets')
